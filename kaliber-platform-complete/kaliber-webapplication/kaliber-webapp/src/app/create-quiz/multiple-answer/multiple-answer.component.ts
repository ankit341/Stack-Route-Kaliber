import { Component, OnInit, OnChanges, Output, EventEmitter, Inject, Input } from '@angular/core';
...
export class MultipleAnswerComponent implements OnInit {
  constructor(@Inject(MAT_DIALOG_DATA) public data,
              private dialogRef: MatDialogRef<MultipleAnswerComponent>,
  ...
  public addcard = [
    {
      ...
      addQuestion: new FormGroup({
        ...
        questionType: new FormControl('MMCQ'),
        maxScore: new FormControl('', [Validators.required, Validators.pattern(/^[0-9 ]*$/)]),
        maxDurationMins: new FormControl('', [Validators.required, Validators.pattern(/^[0-9 ]*$/)]),
  ...
  QuestionComponent: any; visible = true;
  selectable = true;
  removable = true;
  addOnBlur = true;
  ...
  createItem() {
    return this.fb.group({
      optionContent: [''], optionKey: 'dummy', optionReference: 'dummy'


    });
  }
  addOption() {
    this.contentOption = this.addcard[0].addQuestion.get('answerOptions') as FormArray;
  ...
  PutQuiz(questionOption) {
    ...
    this.addcard.push(
      {
        optionCount: [4],
        addQuestion: new FormGroup({
          questionTitle: new FormControl(questionOption.questionTitle, [Validators.required, Validators.pattern(/^[a-zA-Z0-9 ]*$/)]),
          // subject: new FormControl('', [Validators.required, Validators.pattern(/^[a-zA-Z0-9 ]*$/)]),
          ...
          maxScore: new FormControl(questionOption.maxScore, [Validators.required, Validators.pattern(/^[0-9 ]*$/)]),
          maxDurationMins: new FormControl(questionOption.maxDurationMins, [Validators.required, Validators.pattern(/^[0-9 ]*$/)]),
    ...
    this.addcard[0].addQuestion.setControl('answerOptions', this.setExistingAlignment(questionOption.answerOptions));
  ...
  onChange(event, i, j) {
    ...
    this.logger.log('onChange event.checked ' + event.checked, i, j);
    this.logger.log('i', i);
    this.logger.log('j ' + j);
    if (event.checked) {
      this.logger.log('add');
      this.answerKeysOption.push(i.addQuestion.value.answerOptions[j].optionContent);
      this.logger.log(this.answerKeysOption);
    }
    if (!event.checked) {
      this.logger.log('remove');
      const position = this.answerKeysOption.indexOf(i.addQuestion.value.answerOptions[j].optionContent);
      this.answerKeysOption.splice(position, 1);
      this.logger.log('postions: ', position, this.answerKeysOption);
      // this.answerKeysOption.pop(i.addQuestion.value.answerOptions[j].optionContent);
    }
  }
  ...
  openDialog(i): void {
    ...
    i.addQuestion.value.answerKeys = this.answerKeysOption;
    this.quesChild = [];
    // this.logger.log('Question Object ',JSON.stringify(i.addQuestion.value));
  ...
  editQuestion() {
    ...
    // this.addcard[0].addQuestion.value.concepts = this.concepts;
    this.questionService.UpdateQuestion(urlq, this.addcard[0].addQuestion.value, this.questionId).
